(ns seafreightops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (SeaFreightAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the MaritimeFreightGovernor
  ...
  human port/logistics coordinator (or, for a safety concern, the
  captain/harbor master). The approver resumes with `{:approval
  {:status :approved}}` (or :rejected).

  This actor coordinates PORT/LOGISTICS SCHEDULING ONLY -- it never
  navigates a vessel and never overrides a captain's or harbor master's
  safety judgment. See `seafreightops.governor` for the HARD scope
  invariants."
...
(defn build
  ...
  `seafreightops.store/Store`).
  opts:
    :advisor      -- a `seafreightops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; SeaFreightAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; MaritimeFreightGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
      ...
      ;; Approval handoff -- paused by interrupt-before; a human port/
      ;; logistics coordinator (or captain/harbor master for a safety
