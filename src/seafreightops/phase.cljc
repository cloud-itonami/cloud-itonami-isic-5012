(ns seafreightops.phase
  "Phase 0->3 staged rollout for the ISIC-5012 'Sea and coastal freight
  water transport' (cargo/container vessel and coastal-shipping)
  PORT/LOGISTICS SCHEDULING coordination actor.

    Phase 0  read-only              -- no writes, still governor-gated.
    Phase 1  assisted-logging       -- shipment-record logging allowed,
                                       every write needs human approval.
    Phase 2  assisted-coordination  -- adds berth/voyage scheduling and
  ...
  rollout milestone still to come. Flagging a cargo-safety/seaworthiness
  concern always needs a human (captain/harbor master) to actually look
  at it. `seafreightops.governor`'s own `always-escalate-ops` enforces
  the same invariant independently -- two layers, not one, agree on
  this."
  (:require [seafreightops.governor :as governor]))
...
(defn verdict->disposition
  "Map a MaritimeFreightGovernor verdict to a base disposition before
  the phase gate."
  [verdict]
