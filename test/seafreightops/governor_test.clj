(ns seafreightops.governor-test
  "Pure unit tests of `seafreightops.governor/check` against hand-built
  proposals -- the fast, focused complement to `governor-contract-test`'s
  full-graph integration coverage."
  (:require [clojure.test :refer [deftest is testing]]
            [seafreightops.advisor :as adv]
            [seafreightops.governor :as gov]
            [seafreightops.store :as store]))

(def vessel-1 {:vessel-id "vessel-1" :name "MV Harborlight Trader" :registered? true :verified? true})
(def vessel-3 {:vessel-id "vessel-3" :name "MV Pending Survey" :registered? true :verified? false})
(def contractor-1 {:contractor-id "contractor-1" :name "Northgate Marine Maintenance" :registered? true :verified? true})
...
(defn- clean-proposal [op vessel-id]
  {:op op :vessel-id vessel-id :summary "s" :rationale "routine port/logistics coordination"
...
(deftest override-captain-judgment-content-is-hard
  (testing "a proposal touching overriding the captain's safety judgment is HARD-blocked"
    (let [s (store/mem-store {"vessel-1" vessel-1})
          poisoned (assoc (clean-proposal :schedule-berth-operation "vessel-1")
                          :summary "the coordinator overrode the captain's safety judgment to keep the berth window")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest direct-navigation-content-is-hard
  (testing "a proposal touching directly navigating the vessel is HARD-blocked"
    (let [s (store/mem-store {"vessel-1" vessel-1} {"contractor-1" contractor-1})
          poisoned (assoc (clean-maintenance-order "vessel-1" "contractor-1" 100.0)
                          :summary "directly navigated the vessel out of the berth ahead of clearance")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest bypass-maritime-safety-protocol-content-is-hard
  (testing "a proposal touching bypassing a maritime safety protocol is HARD-blocked"
    (let [s (store/mem-store {"vessel-1" vessel-1})
          poisoned (assoc (clean-proposal :log-shipment-record "vessel-1")
                          :rationale "bypassed the maritime safety protocol to save time at the berth")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest legitimate-safety-concern-is-not-scope-excluded
  (testing "flagging observed cargo-load-safety/seaworthiness/hazmat concerns as a SAFETY CONCERN (not a clearance finalization) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"vessel-1" vessel-1})
          concern (assoc (clean-proposal :flag-safety-concern "vessel-1")
                         :value {:concern "hazmat placard mismatch on hold 3 manifest, possible cargo securement anomaly, seaworthiness observation pending survey"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (cargo-load-safety/seaworthiness/hazmat) is exactly what this op exists to surface"))))
...
;; "seaworthiness" or "cargo load safety"), which then accidentally
